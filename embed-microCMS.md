---
description: microCMSのニュースセクションをoutputのHTMLに埋め込む（Vercel Serverless Function経由）
---
...
### Step 2: `output/index.html` を生成

`input/index.html` を読み込み、**そのままの内容で `output/index.html` として書き出して**ください。
（`output/` ディレクトリが存在しない場合は作成してください）

### Step 3: 構造把握

`output/index.html` を読み込み、サイトのセクション構成（id属性一覧）を把握してください。

### Step 4: `output/index.html` の変更
...
### Step 5: `output/css/news.css` を新規作成

既存のCSSファイル（`output/css/about.css` 等）のデザイントーン・CSS変数を参照し、以下のクラスを定義してください:
...
### Step 6: `output/api/news.js` を新規作成（Vercel Serverless Function）
